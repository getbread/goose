...
func RunMigrations(conf *DBConf, migrationsDir string, target int64) (err error) {
	...
	if len(migrations) == 0 {
		...
		fmt.Printf("goose: table name: %s", conf.Table)
		return nil
	}
...
func createVersionTable(conf *DBConf, db *sql.DB) error {
	...
	if _, err := txn.Exec(d.createVersionTableSql(conf.Table)); err != nil {
		txn.Rollback()
		return err
	}
	...
	version := 0
	applied := true
	if _, err := txn.Exec(d.insertVersionSql(conf.Table), version, applied); err != nil {
		txn.Rollback()
		return err
	}
...
func GetEarliestSharedDBVersion(firstPath string, secondPath string) (int64, error) {
	...
	maxLen := fvLen
	downTo := firstVersions[len(firstVersions) - 1]
...
func FinalizeMigration(conf *DBConf, txn *sql.Tx, direction bool, v int64) error {
	...
	stmt := conf.Driver.Dialect.insertVersionSql(conf.Table)
